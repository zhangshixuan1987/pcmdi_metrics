...
# list of test data
mod_cmor = param.mod_cmor
print("mod_cmor:", mod_cmor)
mod_catalogue_json = param.mod_catalogue
if mod_cmor and mod_catalogue_json is not None:
    list_mod = list()
    with open(mod_catalogue_json) as job1:
        mod_catalogue_dict = json.load(job1)
    list_mod = list(mod_catalogue_dict.keys())
    list_mod = sorted(list_mod)
    print("list_mod:", list_mod)
    models = sorted(list_mod)
    print(models)
    # Realizations
    realization = "all"
        ...
        try:
            ...
            try:
                ...
                if list_files is not None:
                    ...
                    if obs_cmor and obs_catalogue_json is not None:
                        if "var_in_file" in obs_catalogue_dict[obs][var_in_file].keys():
                            dict_obs[obs_name][var]["varname"] = obs_catalogue_dict[
                                obs
...
for mod in models:
    ...
    if mod_cmor and mod_catalogue_json is not None:
        runs_list = list(mod_catalogue_dict[mod].keys())
    ...
    for run in runs_list:
        ...
        try:
            for var in list_variables:
                ...
                # -- TEMPORARY --
                if mod_cmor and mod_catalogue_json is not None:
                    var_in_file = mod_catalogue_dict[mod][run][var0]["var_in_file"]
                    period = mod_catalogue_dict[mod][run][var0]["period"]
                    realm = mod_catalogue_dict[mod][run][var0]["CMIP_CMOR_TABLE"]
                    realm2 = realm
                    file_name = (
                        param.modpath.replace("%(tableId)", realm)
                        .replace("%(period)", period)
                        .replace("%(model)", mod)
                        .replace("%(variable)", var0)
                        .replace("%(realization)", run)
                    )
                    landmask_in_file = "sftlf"
                    file_landmask = param.modpath_lf.replace(
                        "%(tableId)", realm
                    ).replace("%(model)", mod)
                    file_areacell = None
                    # print(file_name,file_landmask)
                    # exit()
